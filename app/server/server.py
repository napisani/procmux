...
def start_server(cfg: ProcMuxConfig, 
    ...
    def _start_server():
        class SignalServer(http.server.SimpleHTTPRequestHandler):
            ...
            def handle_stop_by_name(self):
                ...
                if process:
                    terminal_controller = terminal_controllers.get(process.index)
            ...
            def _wait_for_process_stop(self, terminal_controller: TerminalController):
                start_time = time()
            ...
            def handle_stop_running(self):
                running_processes = [p for p in process_state.process_list if p.running]
                for process in running_processes: 
                    terminal_controller = terminal_controllers.get(process.index)
                    if terminal_controller:
                        terminal_controller.stop_process()
        ...
        with socketserver.TCPServer((cfg.signal_server.host, cfg.signal_server.port), 
