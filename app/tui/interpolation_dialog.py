...
class InterpolationDialog():
    ...
    def __init__(
        self, process: Process, run_in_background: bool, config: ProcMuxConfig,
        float_container: FloatContainer,
        on_finished: Callable[[Process, bool, Optional[List[Interpolation]]],
                              None]):
        ...
        self._text_inputs = [
            TextArea(
                height=1,
                width=50,
                prompt=self._config.layout.field_replacement_prompt.replace(
                    '__FIELD_NAME__', interp.field),
        ...
        self._focusable_components = [
            *self._text_inputs, start_button, cancel_button
        ]

        self._container = HSplit([
            Box(body=HSplit(children=self._text_inputs, padding=1),
                padding=Dimension(min=1, max=1, preferred=1),
                padding_bottom=0),
            Box(body=VSplit(children=[start_button, cancel_button],
    ...
    def _on_start(self):
        ...
        for input_field, interp in zip(self._text_inputs,
                                       self._interpolations):
            final_input_interpolations.append(
                Interpolation(field=interp.field,
                              value=input_field.text,
                              default_value=interp.default_value))
        self._float_container.floats = []
        self._on_finished(self._process, self._run_in_background,
    ...
    def _move_focus(self, direction: int):
        self._tab_ix = (self._tab_ix + direction) % len(
            self._focusable_components)
    ...
    def get_keybindings(self) -> DocumentedKeybindings:
        ...
        def next_input():
            logger.info(
                'interpolation dialog next_input - focusing on next tab index')
            self._move_focus(1)

        def previous_input():
            logger.info(
                'interpolation dialog previous_input - focusing on next tab index'
            )
            self._move_focus(-1)

        kb = DocumentedKeybindings()
        kb.register_configured_keybinding_sans_event(
            self._config.keybinding.next_input, next_input, 'next input')
        kb.register_configured_keybinding_sans_event(
            self._config.keybinding.previous_input, previous_input,
            'previous input')
        kb.register_configured_keybinding_sans_event(
            self._config.keybinding.submit_dialog, self._on_start, 'start')
        kb.register_configured_keybinding_sans_event(
            self._config.keybinding.cancel_dialog, self._on_cancel, 'cancel')
        return kb
    ...
    def start_interpolation(self):
        ...
        if app and self._text_inputs:
            self._float_container.floats = [
                Float(Frame(self._container, self._process.name))
            ]
