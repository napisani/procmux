...
parser_signal_start = sub_parsers.add_parser('signal-start', help='send a start signal to processes managed by a running procmux instance')
parser_signal_start.add_argument('--name', type=str, 
...
parser_signal_stop = sub_parsers.add_parser('signal-stop', help='send a stop signal to processes managed by a running procmux instance')
parser_signal_stop.add_argument('--name', type=str, help='the process name to send the signal to', required=True)
parser_signal_stop.add_argument('--config', required=False)
parser_signal_stop.add_argument('--config-override', required=False)

parser_signal_restart = sub_parsers.add_parser('signal-restart', help='send a restart signal to processes managed by a running procmux instance')
parser_signal_restart.add_argument('--name', type=str, help='the process name to send the signal to', required=True)
...
parser_signal_restart_running = sub_parsers.add_parser('signal-restart-running', help='send a restart signal to all running processes managed by a running procmux instance')
...
parser_signal_stop_running = sub_parsers.add_parser('signal-stop-running', help='send a stop signal to all running processes managed by a running procmux instance')
...
if len(sys.argv) == 1 or sys.argv[1] not in ['start', 'signal-start', 'signal-stop', 'signal-restart', 'signal-restart-running', 'signal-stop-running']:
